...
func setupDataChannelHandler(pc *webrtc.PeerConnection) {
	...
	/* This deadlocks?
	if server_dc, err := pc.CreateDataChannel("server-opened-channel", nil); err == nil {
		server_dc.OnOpen(func() {
			if sendErr := server_dc.SendText("Server opened channel ready"); sendErr != nil {
