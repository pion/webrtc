...
func newAcceptTestPeer(t *testing.T, stuckSID uint16, openTimeout time.Duration) *acceptTestPeer {
	...
	go transport.acceptDataChannels(res.assoc)
...
func TestSCTPTransportAcceptDataChannelsStreamEOF(t *testing.T) {
	// The GizClaw/pion-sctp fork answers a retransmitted outgoing reset
	// request with the cached "In progress" response and does not re-check
	// pending resets when I-FORWARD-TSN closes the TSN gap, so the peer's
	// reset below never completes. Re-enable once the fork is fixed.
	t.Skip("blocked on GizClaw/pion-sctp pending stream reset after I-FORWARD-TSN")
