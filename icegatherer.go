...
func (g *ICEGatherer) createAgent() error {
	...
	parameters := ICEParameters{
		UsernameFragment: g.api.settingEngine.candidates.UsernameFragment,
		Password:         g.api.settingEngine.candidates.Password,
	}
	if parameters.UsernameFragment == "" {
		parameters.UsernameFragment, err = ice.GenerateUFrag()
		if err != nil {
			return err
		}
	}
	if parameters.Password == "" {
		parameters.Password, err = ice.GeneratePwd()
		if err != nil {
			return err
		}
	}
...
func (g *ICEGatherer) gather(restart bool) error { //nolint:cyclop
	...
	parameters, err := g.GetLocalParameters()
	if err != nil {
		return err
	}
	if restart {
		parameters.UsernameFragment = g.api.settingEngine.candidates.UsernameFragment
		parameters.Password = g.api.settingEngine.candidates.Password
	}

	g.lock.RLock()
	defer g.lock.RUnlock()

	networkTypes := g.api.settingEngine.candidates.ICENetworkTypes
	if len(networkTypes) == 0 {
		networkTypes = supportedNetworkTypes()
	}
	options := []ice.GatherOption{
		ice.WithURLs(g.validatedServers),
		ice.WithNetworkTypes(toICENetworkTypes(networkTypes)),
		ice.WithLocalCredentials(parameters.UsernameFragment, parameters.Password),
	}
